...
const submit = async () => {
    ...
    if (await updateJob(route.params.id, data)) {
        router.push('/jobs')
    }
}
...
onMounted(async () => {
    ...
    formData.title = job.value.title
    formData.description = job.value.description
    formData.content = job.value.content
    formData.start_at = formatDate(new Date(job.value.start_at), 'YYYY-MM-DD')
    formData.end_at = formatDate(new Date(job.value.end_at), 'YYYY-MM-DD')
    formData.published_at = formatDate(new Date(job.value.published_at), 'YYYY-MM-DD')
    // formData.company = job.value.company
    formData.job_category = job.value.job_category
    // formData.position = job.value.position
